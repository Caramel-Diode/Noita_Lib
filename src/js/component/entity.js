...
component.entity = class extends component.base {
    ...
    static getDataSection = (() => {
        ...
        return /** @param {DB.entity} entityData */ async entityData => {
            ...
            const dd = pc.offeredDamage; //简写伤害数据 damageData
            ...
            //#region 简单数据加载
            if (dd.projectile !== 0) loadAttr("projectileDamage", dd.projectile, tbody);
            if (dd.melee !== 0) loadAttr("meleeDamage", dd.melee, tbody);
            if (dd.electricity !== 0) loadAttr("electricityDamage", dd.electricity, tbody);
            if (dd.fire !== 0) loadAttr("fireDamage", dd.fire, tbody);
            if (dd.explosion !== 0) loadAttr("explosionDamage", dd.explosion, tbody);
            if (dd.ice !== 0) loadAttr("iceDamage", dd.ice, tbody);
            if (dd.slice !== 0) loadAttr("sliceDamage", dd.slice, tbody);
            if (dd.healing !== 0) loadAttr("healingDamage", dd.healing, tbody);
            if (dd.curse !== 0) loadAttr("curseDamage", dd.curse, tbody);
            if (dd.holy !== 0) loadAttr("holyDamage", dd.holy, tbody);
            if (dd.drill !== 0) loadAttr("drillDamage", dd.drill, tbody);
            if (dd.radioactive !== 0) loadAttr("radioactiveDamage", dd.radioactive, tbody);
