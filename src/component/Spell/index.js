...
const Spell = (() => {
    ...
    return Object.freeze(
        class extends Base {
            static queryById = id => db_spell.queryById(id);
            static queryByName = name => db_spell.queryByName(name);
            static queryByExp = exp => db_spell.queryByExp(exp);
            static observedAttributes = [...super.observedAttributes, "spell.id", "spell.name", "spell.exp", "spell.remain"];
            static {
                const superStyleSheets = super.prototype.publicStyleSheets;
                this.prototype.publicStyleSheets = {
                    /** @type {Array<CSSStyleSheet>} 图标模式样式表 */ icon: [...superStyleSheets.icon, gss(embed(`#icon.css`))],
                    /** @type {Array<CSSStyleSheet>} 面板模式样式表 */ panel: [...superStyleSheets.panel, gss(embed(`#panel.css`))]
                };
            ...
            /** @type {Array<db_spell>} */ spellDatas = [];
            ...
            /**
             * 获取投射物数据表
             * @param {db_spell} spellData 投射物数据
            ...
            static async getDataSection(spellData) {
                ...
                for (let i = 0; i < sd.offeredProjectiles.length; i++) {
                    ...
                    const section_ = await Entity.getDataSection(projectileData);
                ...
                baseLoader._default("type", typeInfoMap.get(sd.type)[0]); // 法术类型
            ...
            static #addPanelDataSwitchFeaturesTo = (() => {
                const main = event => {
                    /** @type {HTMLLIElement} */
                    let li;
                 ...
                 */
                const byMouse = event => main(event);
                /**
                 * 用于键盘触发 `Enter`
                 * @param {KeyboardEvent} event
                 */
                const byKeyboard = event => {
                 ...
                 */
                return target => {
                    target.addEventListener("click", byMouse);
                    target.addEventListener("keydown", byKeyboard);
                ...
                if (this.#currentDataIndex !== -1) {
                    ...
                    if (this.spellDatas.length > 1) {
                        ...
                        for (let i = 0; i < this.spellDatas.length; i++) {
                            ...
                            Spell.#addPanelDataSwitchFeaturesTo(li);
                            ol.append(li);
                        }
            ...
            contentUpdate() {
                ...
                if (spellId) this.spellDatas = [db_spell.queryById(spellId)];
                else {
                    const spellName = this.getAttribute("spell.name");
                    if (spellName) this.spellDatas = [db_spell.queryByName(spellName)];
                    else {
                        const spellExp = this.getAttribute("spell.exp");
                        if (spellExp) this.spellDatas = db_spell.queryByExp(spellExp);
                        if (this.spellDatas.length === 0) this.spellDatas = [db_spell.$NULL];
                    }
                }
